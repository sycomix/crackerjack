...
def process_1x_database(data, databaseName, maxInlineSize=1024):
    ...
    return f"$keepass$*1*{keyTransfRounds}*{algorithm}*{finalRandomseed}*{transfRandomseed}*{encIV}*{contentsHash}*{end}"
...
def process_2x_database(data, databaseName):
    ...
    return f"$keepass$*2*{transformRounds}*{dataStartOffset}*{masterSeed}*{transformSeed}*{initializationVectors}*{expectedStartBytes}*{firstEncryptedBytes}"
...
def process_database(filename):
    ...
    fileSignature = hexlify(data[:8])

    if fileSignature in [b'03d9a29a67fb4bb5', b'03d9a29a66fb4bb5']:
        # "2.X"
